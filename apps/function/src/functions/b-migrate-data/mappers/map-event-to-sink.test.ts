...
describe('mapEventToSink', () => {
	...
	test('returns empty object for in_house event type', () => {
		const result = mapEventToSink({
			eventType: 'in_house',
			eventStartDateTime: '2024-03-01T10:00:00Z'
		});
	...
	test('calls callback for unmapped event types', () => {
		let capturedEventType;

		mapEventToSink({ eventType: 'unknown_event_type', eventStartDateTime: '2024-03-01T10:00:00Z' }, (eventType) => {
			capturedEventType = eventType;
		});

		assert.strictEqual(capturedEventType, 'unknown_event_type');
	});
});
