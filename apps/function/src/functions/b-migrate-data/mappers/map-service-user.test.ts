...
describe('mapServiceUsersToAppealRelations', () => {
	...
	test('uses first appellant when multiple present', () => {
		const serviceUsers = [
		...
		assert.ok(result.appellant);
		assert.strictEqual(result.appellant.firstName, 'First');
		assert.strictEqual(result.appellant.email, 'first@example.com');
	});

	test('ignores unknown service user types', () => {
		const serviceUsers = [
