...
export function mapServiceUsersToAppealRelations(
	serviceUsers: AppealServiceUser[],
	onUnknownType?: (serviceUserType: string | null | undefined) => void
): {
	...
	for (const serviceUser of serviceUsers) {
		...
		if (category === null && serviceUser.serviceUserType) {
			if (onUnknownType) {
				onUnknownType(serviceUser.serviceUserType);
			}
			continue;
		}

		if (category === SERVICE_USER_TYPE.APPELLANT && !result.appellant) {
		...
		if (category === SERVICE_USER_TYPE.AGENT && !result.agent) {
