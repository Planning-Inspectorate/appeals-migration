resource "azurerm_redis_cache" "manage" {
...
resource "azurerm_private_endpoint" "redis_manage" {
  ...
  private_service_connection {
    ...
    private_connection_resource_id = azurerm_redis_cache.manage.id
...
resource "azurerm_key_vault_secret" "redis_web_connection_string" {
  ...
  value        = azurerm_redis_cache.manage.primary_connection_string
