...
resource "azurerm_cdn_frontdoor_custom_domain" "web" {
  name                     = "${local.org}-fd-${local.service_name}-web-${var.environment}"
  cdn_frontdoor_profile_id = data.azurerm_cdn_frontdoor_profile.shared.id
  host_name                = var.apps_config.manage.domain
  provider                 = azurerm.front_door
...
resource "azurerm_cdn_frontdoor_firewall_policy" "web" {
  ...
  custom_rule {
    ...
    enabled                        = var.front_door_config.waf_rate_limits.enabled
    priority                       = 100
    rate_limit_duration_in_minutes = var.front_door_config.waf_rate_limits.duration_in_minutes
    rate_limit_threshold           = var.front_door_config.waf_rate_limits.threshold
    type                           = "RateLimitRule"
    action                         = "Block"
