resource "azurerm_storage_account" "sql_server" {
  ...
  name                             = "pinsstsqlamigrat${var.environment}"
...
resource "azurerm_mssql_server_security_alert_policy" "sql_server" {
  state                      = var.monitoring_config.alerts_enabled ? "Enabled" : "Disabled"
  resource_group_name        = azurerm_resource_group.primary.name
...
resource "azurerm_mssql_server_vulnerability_assessment" "appeals_migration_sql_server" {
  count = var.monitoring_config.alerts_enabled ? 1 : 0
...
resource "azurerm_monitor_metric_alert" "sql_db_cpu_alert" {
  ...
  action {
    action_group_id = azurerm_monitor_action_group.appeals_migration_tech.id
  }
