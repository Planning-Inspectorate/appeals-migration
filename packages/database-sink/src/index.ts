...
import { PrismaClient } from '@pins/manage-appeals-database/src/client/client.ts';
import { PrismaMssql } from '@prisma/adapter-mssql';
import type { Logger } from 'pino';

export type { PrismaClient };

export function initManageAppealsDatabaseClient(
	config: { database: string; NODE_ENV: string },
	logger: Logger
): PrismaClient {
	...
	return newManageAppealsDatabaseClient(config.database, prismaLogger);
}

export function newManageAppealsDatabaseClient(connectionString: string, logger?: Logger): PrismaClient {
